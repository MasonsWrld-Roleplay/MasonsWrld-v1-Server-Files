...
local function getNewCharacterConfig()
    ...
    config.tattoos      = not Config.VMSTattooShop and Config.NewCharacterSections.Tattoos
...
local function OpenClothingShop(isPedMenu)
    ...
    if isPedMenu then
        ...
        config.tattoos = not Config.VMSTattooShop and true
    end
...
RegisterNetEvent("illenium-appearance:client:ClearStuckProps2", function()
    if InCooldown() or Framework.CheckPlayerMeta() then
        return
    end
