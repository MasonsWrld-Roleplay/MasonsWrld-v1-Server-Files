...
RegisterNetEvent('Pug:Server:GetJobStats', function(Type)
    ...
    if Framework == "QBCore" then
        ...
        if Type == "balance" then
            ...
            local bankBalances = MySQL.Sync.fetchAll('SELECT account_name, account_balance FROM bank_accounts WHERE account_name IN (' .. jobNamesStr .. ')')
            
            for _, row in ipairs(bankBalances) do
                if jobStats[row.account_name] then
                    jobStats[row.account_name].bank_balance = row.account_balance
                end
            end
        end
    elseif Framework == "ESX" then
