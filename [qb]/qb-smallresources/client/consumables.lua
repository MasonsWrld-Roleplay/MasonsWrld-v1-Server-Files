...
RegisterNetEvent('consumables:client:drinklight', function()
    alcoholCount += 1
    AlcoholLoop()
    if alcoholCount > 1 and alcoholCount < 4 then
        TriggerEvent('evidence:client:SetStatus', 'alcohol', 200)
...
RegisterNetEvent('consumables:client:drinkheavy', function()
    alcoholCount += 2
    AlcoholLoop()
    if alcoholCount > 1 and alcoholCount < 4 then
        TriggerEvent('evidence:client:SetStatus', 'alcohol', 200)
...
--     local Heal = Health + 15
--     SetEntityHealth(Player, Heal)
-- end
...
RegisterNetEvent('consumables:client:UseIbuprofen', function(source)
    if lib.progressBar({
        duration = 3000,
    ...
    then TriggerServerEvent('consumables:server:Ibuprofen') else end
    end)

RegisterNetEvent('consumables:client:IboprufenUsed', function()
...
RegisterNetEvent('consumables:client:UseIfak', function(source)
    if lib.progressBar({
        duration = 8000,
        label = 'Using IFAK',
        useWhileDead = false,
    ...
    then TriggerServerEvent('consumables:server:Ifak') else end
    end)

RegisterNetEvent('consumables:client:IfakUsed', function()
    local Player = PlayerPedId()
    local Health = GetEntityHealth(Player)
    local Heal = Health + 40
    SetEntityHealth(Player, Heal)
end)
