...
CreateThread(function()
    while true do
        ...
        if GetSelectedPedWeapon(PlayerPedId()) == weaponHash then
            SetWeaponDamageModifier(weaponHash, damageReduce / 100)
        else
...
local weaponHash = GetHashKey("WEAPON_KNIFE")
local damageReduce = 30
...
local weaponHash = GetHashKey("WEAPON_HATCHET")
local damageReduce = 15
...
local weaponHash = GetHashKey("weapon_screwdriver")
local damageReduce = 60
...
local weaponHash = GetHashKey("weapon_kitchenknife")
local damageReduce = 30
...
local weaponHash = GetHashKey("weapon_rustyshank")
local damageReduce = 30
