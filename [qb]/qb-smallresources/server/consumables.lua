...
RegisterNetEvent('consumables:server:Ibuprofen', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    exports.ox_inventory:RemoveItem(source, 'ibuprofen', 1)
    TriggerClientEvent('consumables:client:IboprufenUsed', source)
end)

RegisterNetEvent('consumables:server:Ifak', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    exports.ox_inventory:RemoveItem(source, 'ifak', 1)
    TriggerClientEvent('consumables:client:IfakUsed', source)
end)
