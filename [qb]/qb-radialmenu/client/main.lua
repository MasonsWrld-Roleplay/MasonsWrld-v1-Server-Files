...
local function IsPoliceOrEMS()
    return (PlayerData.job.name == "police" or PlayerData.job.type == "leo")
end
    ...
    if (IsDowned() and IsPoliceOrEMS()) then
        FinalMenuItems = {
            [1] = {
                ...
                type = 'client',
                event = 'cd_dispatch:PDPanicButtonEvent',
                shouldClose = true,
            },
        }
    else
...
RegisterKeyMapping('radial', Lang:t("general.command_description"), 'keyboard', Config.Keybind)



-- Exectute Commands

RegisterNetEvent('RadialMenu:Client:Trap', function()
    ExecuteCommand("trap")
end)

RegisterNetEvent('RadialMenu:Client:Steal', function()
    ExecuteCommand("steal")
end)

RegisterNetEvent('RadialMenu:Client:Jail', function()
    ExecuteCommand("jailcp")
end)

RegisterNetEvent('RadialMenu:Client:MDT', function()
    ExecuteCommand("mdt")
end)
