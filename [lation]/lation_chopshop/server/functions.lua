...
--- @param bone string Specific bone name: bonnet, door_dside_f, door_pside_f, door_dside_r, door_pside_r, boot, wheel_lf, wheel_rf, wheel_lr, wheel_rr, frame
ChoppedPart = function(source, model, type, bone)
...
RegisterNetEvent('lation_chopshop:phoneMessage', function(source, data, number)
    ...
    elseif Config.Phone == 'gksphone' then
        local identifier = GetIdentifier(source)
        local messsage = { sender = data.sender, image = '/html/static/img/icons/mail.png', subject = data.subject, message = data.content }
        exports["gksphone"]:SendNewMailOffline(identifier, messsage)
...
PoliceCount = function()
    ...
    elseif Framework == 'qb' then
        for _, players in pairs(QBCore.Functions.GetPlayers()) do
            ...
            for _, jobs in pairs(Config.Police.jobs) do
                local jobNames = jobs
                if job.name == jobNames and job.onduty then
                    jobCount = jobCount + 1
                end
            end
        end
