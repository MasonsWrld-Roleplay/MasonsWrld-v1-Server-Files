...
local using = false
RegisterNetEvent('drc_koi:consumables', function(text, animation, duration, effect)
    if not using then
        using = true
...
Action = function(data)
    if Config.Framework == "ESX" then
        if tostring(data.status) == "drunk" then
            ...
            TriggerServerEvent("drc_koi:server:add", tostring(data.status), tonumber(data.add))
        end
    elseif Config.Framework == "qbcore" then
        if tostring(data.status) == "drunk" then
            TriggerEvent("evidence:client:SetStatus", "alcohol", 200)
...
OpenStash = function(stash)
    ...
    local data = nil
    for _, stashData in pairs(Config.koi.Stashes) do
...
OpenCloakRoomMenu = function()
    lib.registerContext({
        ...
        options = {
            ['Outfits'] = {
                arrow = false,
                event = 'drc_koi:outfits'
            },
            ['Put on job outfit'] = {
                arrow = false,
                event = 'drc_koi:koijoboutfit'
            },
            ['Put on civil outfit'] = {
                arrow = false,
                event = 'drc_koi:civiloutfit'
            }
        }
    })
    lib.showContext('koi_cloakroom')
end

RegisterNetEvent("drc_koi:civiloutfit", function()
...
RegisterNetEvent("drc_koi:koijoboutfit", function()
...
RegisterNetEvent("drc_koi:outfits", function()
    if Config.Clothing == "esx_skin" then
        ESX.TriggerServerCallback('drc_koi:getPlayerDressing', function(dressing)
            local elements = {}
            ...
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
                ...
                TriggerEvent('skinchanger:getSkin', function(skin)
                    ESX.TriggerServerCallback('drc_koi:getPlayerOutfit', function(clothes)
...
function SpawnObjects()
    CreateThread(function()
        Register = CreateObject(`v_ret_gc_cashreg`, -1033.86, -1483.04, 4.61, false, false,false)
        SetEntityHeading(Register, 0 + 0)
        FreezeEntityPosition(Register, true)
    end)
    CreateThread(function()
        Deserter = CreateObject(`v_res_tre_mixer`, -1076.68, -1449.67, -1.52, false, false,false)
        SetEntityHeading(Deserter, 180.0)
        FreezeEntityPosition(Deserter, true)
    end)
    CreateThread(function()
        AlertButton = CreateObject(`hei_prop_bank_alarm_01`, -1075.6558837890625, -1442.419677734375, -1.05735278129577, - 1.0, false, false,false)
        SetEntityHeading(AlertButton, 31.92)
        FreezeEntityPosition(AlertButton, true)
    end)
    CreateThread(function()
        Phone = CreateObject(`v_res_fa_phone`, -1065.69, -1443.10, -1.95, false, false,false)
        SetEntityHeading(Phone, 124.13)
    ...
    if Config.Dispatch.enabled then
        if Config.Dispatch.script == "cd_dispatch" then
            ...
            TriggerServerEvent('cd_dispatch:AddNotification', {
                ...
                message = "Trespassing reported in koi cafe by citizen",
                flash = 0,
        ...
        elseif Config.Dispatch.script == "linden_outlawalert" then
            local data = { displayCode = "10-18", description = "Trespassing", isImportant = 1,
                ...
                length = '10000', infoM = 'fa-info-circle', info = "Trespassing reported in koi cafe by citizen" }
