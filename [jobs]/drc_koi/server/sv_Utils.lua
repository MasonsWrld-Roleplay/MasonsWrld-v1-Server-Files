...
CreateThread(function()
    if GetResourceState(GetCurrentResourceName()) == 'started' then
        print('DRC KOI STARTED ON VERSION: ' .. ver)
    end
end)
...
if Config.Framework == "ESX" then
    if Config.NewESX then
        ...
        TriggerEvent('esx_society:registerSociety', 'koi', 'koi', 'society_koi', 'society_koi',
            'society_koi', { type = 'private' })
                    ...
                    local embed = {
                        {
                            ...
                            ["description"] = message,
                            ["footer"] = {
                                ["text"] = "Logs by DRC SCRIPTS for DRC koi!",
                            },
                        }
                    }
                    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST',
                        json.encode({ username = "DRC koi", embeds = embed,
    ...
    RegisterUsable(k, function(source)
        ...
        if v.Remove then
            if GetItem(v.RemoveItem, v.RemoveItemCount, src) then
                ...
                TriggerClientEvent('drc_koi:consumables', src, v.ProgressBar, v.animation, v.duration, v.Effect)
            end
        else
            if v.Add then
                AddItem(v.AddItem, v.AddItemCount, src)
...
AddEventHandler('onServerResourceStart', function(resourceName)
    if resourceName == 'ox_inventory' or resourceName == GetCurrentResourceName then
        for _, v in pairs(Config.koi.Stashes) do
...
if Config.Clothing == "esx_skin" then
    ESX.RegisterServerCallback('drc_koi:getPlayerDressing', function(source, cb)
        local xPlayer = ESX.GetPlayerFromId(source)
    ...
    ESX.RegisterServerCallback('drc_koi:getPlayerOutfit', function(source, cb, num)
        local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('drc_koi:server:add', function(status, amount)
