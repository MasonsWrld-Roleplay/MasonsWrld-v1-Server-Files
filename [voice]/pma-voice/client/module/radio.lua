				...
				while radioPressed do
					...
					if shouldPlayAnimation and HasAnimDictLoaded("ultra@walkie_talkie") then
						if not IsEntityPlayingAnim(PlayerPedId(), "ultra@walkie_talkie", "walkie_talkie", 3) then
							TaskPlayAnim(PlayerPedId(), "ultra@walkie_talkie", "walkie_talkie", 8.0, 2.0, -1, 50, 2.0, false,
								false,
							false)
							DisableControlAction(2, 24, true)
							DisableControlAction(2, 25, true)
						if not IsEntityAttachedToEntity(object, player) then
							while not HasModelLoaded(prop) do
								RequestModel(prop)
								Citizen.Wait(100)
								end
							object = CreateObject(prop, GetEntityCoords(PlayerPedId()), true)
							AttachEntityToEntity(object, player, GetPedBoneIndex(player, 18905), 0.13, 0.0300, 0.020, -90.0000, 0.0, -59.9999, true, true, false, true, 1, true)
							end
						end
					end
...
RegisterCommand('-radiotalk', function()
	...
	if radioChannel > 0 and radioPressed then
		...
		EnableControlAction(2, 25, true)
		if IsEntityAttachedToEntity(object, player) then
			DeleteEntity(object)
		end
