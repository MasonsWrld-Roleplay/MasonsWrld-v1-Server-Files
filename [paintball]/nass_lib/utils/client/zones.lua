function controlCamera(cam)
    ...
    if IsDisabledControlPressed(0, Config.controls.speed.faster.control) then
        speedMultiplier = 60.0
    elseif IsDisabledControlPressed(0, Config.controls.speed.slower.control) then
        speedMultiplier = 15.0
    end

    if IsDisabledControlPressed(0, Config.controls.z.up.control) then
        camPos = camPos + (upDir * (speedMultiplier * frameTime))
    elseif IsDisabledControlPressed(0, Config.controls.z.down.control) then
        camPos = camPos - (upDir * (speedMultiplier * frameTime))
    end

    if IsDisabledControlPressed(0, Config.controls.x.fwd.control) then
        camPos = camPos + (fwdDir * (speedMultiplier * frameTime))
    elseif IsDisabledControlPressed(0, Config.controls.x.back.control) then
        camPos = camPos - (fwdDir * (speedMultiplier * frameTime))
    end

    if IsDisabledControlPressed(0, Config.controls.y.right.control) then
        camPos = camPos + (rightDir * (speedMultiplier * frameTime))
    elseif IsDisabledControlPressed(0, Config.controls.y.left.control) then
        camPos = camPos - (rightDir * (speedMultiplier * frameTime))
    end
...
function nass.pickLocation(options)
    ...
    local scaleform = nass.createInstructionalMessageScaleform({
        {'Cancel', Config.controls.input.cancel.control},
        {'Select', Config.controls.input.select.control},
        {'Rotation', Config.controls.input.rotDown.control},
        {'Slower', Config.controls.speed.slower.control},
        {'Faster', Config.controls.speed.faster.control},
        {'Down', Config.controls.z.down.control},
        {'Up', Config.controls.z.up.control},
        --[[{'Right', Config.controls.y.right.control},
        {'Left', Config.controls.y.left.control},
        {'Back', Config.controls.x.back.control},
        {'Forward', Config.controls.x.fwd.control},]]
    })
    ...
    while true do
        ...
        if (#(newPos - plyPos) > 300.0 and not options.ignoreDistance) or IsDisabledControlJustPressed(0, Config.controls.input.cancel.control) then
        ...
        if IsDisabledControlJustPressed(0, Config.controls.input.select.control) then
        ...
        if options.heading or options.car or options.prop then
            if IsDisabledControlPressed(0, Config.controls.input.rotUp.control) then
                dirHeading = dirHeading + (500.0 * GetFrameTime())
            elseif IsDisabledControlPressed(0, Config.controls.input.rotDown.control) then
                dirHeading = dirHeading - (500.0 * GetFrameTime())
            end
...
function nass.editZone(inputPoints)
    ...
    local scaleform = nass.createInstructionalMessageScaleform({
        {'Done', Config.controls.input.cancel.control},
        {'Undo', Config.controls.input.undo.control},
        {'Select', Config.controls.input.select.control},
        {'Slower', Config.controls.speed.slower.control},
        {'Faster', Config.controls.speed.faster.control},
        {'Down', Config.controls.z.down.control},
    ...
    while true do
        ...
        if IsDisabledControlJustPressed(0, Config.controls.input.cancel.control) then
            SetCamActive(cam, false)
            RenderScriptCams(false, false, 0, true, false)
            DestroyCam(cam, false)
        ...
        if IsDisabledControlJustPressed(0, Config.controls.input.select.control) then
            ...
            local newPoint = vector3(hitCoords.x, hitCoords.y, zCoord)
        ...
        if IsDisabledControlJustPressed(0, Config.controls.input.undo.control) and #points > 0 then
